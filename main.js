	...
	Game.Init=function()
	{
		...
		Game.resize=function()
		{
			...
			*/
			...
			var scale=Math.min(
				w/Math.max(500*zoom,w),
				h/Math.max(125*zoom,h)
			);
			scale*=zoom;
		...
		Game.tooltip.update=function()
		{
			...
			// 画面下部に固定配置、横方向は中央に配置
			var X=(Game.windowW-width)/2;
			X=Math.max(0,Math.min(Game.windowW-width-16,X-8));
		...
		{
			//touch events
			AddEvent(bigCookie,'touchstart',function(e){
				Game.BigCookieState=1;
				Game.ClickCookie(e);
				if (Game.prefs.cookiesound) {Game.playCookieClickSound();}
				e.preventDefault();
			});
			AddEvent(bigCookie,'touchend',function(e){
			...
			AddEvent(document,'touchend',function(e){Game.lastActivity=Game.time;Game.mouseDown=0;Game.Click=1;});
		}
		...
		Game.UpdateMenu=function()
		{
			...
			// Copy menu content to appropriate mobile container based on menu type
			if (Game.onMenu === 'prefs') {
				var menuContainer = l('menuContainer');
				if (menuContainer) {
					menuContainer.innerHTML=str;
				}
			} else if (Game.onMenu === 'stats') {
				var statsContainer = l('statsContainer');
				if (statsContainer) {
			...
			if (App)
			{
				...
				// Also apply the same to appropriate mobile container
				if (Game.onMenu === 'prefs') {
					var menuContainer = l('menuContainer');
					if (menuContainer) {
						var containerAnchors=menuContainer.getElementsByTagName('a');
						for (var i=0;i<containerAnchors.length;i++)
						{
							var it=containerAnchors[i];
							if (it.href)
							{
								AddEvent(it,'click',function(href){return function(){
									App.openLink(href);
					...
					var statsContainer = l('statsContainer');
					if (statsContainer) {
						var statsAnchors=statsContainer.getElementsByTagName('a');
						for (var i=0;i<statsAnchors.length;i++)
						{
							var it=statsAnchors[i];
							if (it.href)
							{
								AddEvent(it,'click',function(href){return function(){
									App.openLink(href);
		...
		Game.Object=function(name,commonName,desc,icon,iconColumn,art,price,cps,buyFunction)
		{
			...
			else
			{
				...
				this.toResize=true;
				var me=this;
				setTimeout(function(){me.toResize=true;},0);
